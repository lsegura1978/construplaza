    ...
    Private Sub btningresar_Click(sender As Object, e As EventArgs) Handles btningresar.Click
        ...
        Try
            ...
            With cmdconsulta
                .Connection = conexion
                .CommandText = "SELECT " +
                                "C1.TOTAL_BENEFICIOS-C1.TOTAL_DEDUCCIONES AS TOTAL " +
                                "FROM " +
                                "(SELECT " +
                                "EMP.EMPLEADO " +
                                ",EMP.NOMBRE " +
                                ",CASE WHEN EMP.NOMINA='QUIN' THEN (SELECT SUM(C1.TOTAL/4)AS TOTALES " +
                                ...
                                "WHERE (NH.NUMERO_NOMINA IN (SELECT TOP (4) NUMERO_NOMINA " +
                                ...
                                "C.CONCEPTO LIKE '%DQ%'))C1 WHERE C1.EMPLEADO=EMP.EMPLEADO AND (C1.HORAS > 0 OR C1.TOTAL > 0) " +
                                "AND C1.CONCEPTO LIKE 'D%') " +
                                "ELSE " +
                                "(SELECT SUM(C1.TOTAL/4)AS TOTALES " +
                                "FROM (SELECT E.EMPLEADO,E.NOMBRE,U.DESCRIPCION AS UBICACION,C.CONCEPTO,C.DESCRIPCION,ECN.CANTIDAD AS HORAS,ECN.TOTAL,NH.FECHA_INICIO,NH.FECHA_FIN " +
                                "FROM  EXACTUS.digema.EMPLEADO AS E WITH(NOLOCK) " +
                                "INNER JOIN EXACTUS.digema.EMPLEADO_CONC_NOMI AS ECN WITH(NOLOCK) ON ECN.EMPLEADO = E.EMPLEADO " +
                                "INNER JOIN EXACTUS.digema.UBICACION U  ON E.UBICACION= U.UBICACION " +
                                ...
                                "C.CONCEPTO LIKE '%DN%'))C1 WHERE C1.EMPLEADO=EMP.EMPLEADO AND (C1.HORAS > 0 OR C1.TOTAL > 0) " +
                                "AND C1.CONCEPTO LIKE 'D%') " +
                                "End " +
                                ...
                                "INNER JOIN EXACTUS.DIGEMA.NOMINA N WITH(NOLOCK) ON N.NOMINA=NH.NOMINA AND N.NOMINA='QUIN' AND NH.FECHA_APROBACION IS NOT NULL " +
                                "ORDER BY FECHA_FIN DESC)) AND (C.CONCEPTO LIKE '%BQ%' OR " +
                                "C.CONCEPTO LIKE '%DQ%'))C1 WHERE C1.EMPLEADO=EMP.EMPLEADO AND (C1.HORAS > 0 OR C1.TOTAL > 0) AND C1.CONCEPTO LIKE 'B%' " +
                                ") ELSE " +
                                "(SELECT SUM(C1.TOTAL/4)AS TOTALES " +
                                "FROM (SELECT E.EMPLEADO,E.NOMBRE,U.DESCRIPCION AS UBICACION,C.CONCEPTO,C.DESCRIPCION,ECN.CANTIDAD AS HORAS,ECN.TOTAL,NH.FECHA_INICIO,NH.FECHA_FIN " +
                                "FROM  EXACTUS.digema.EMPLEADO AS E WITH(NOLOCK) " +
                                ...
                                "INNER JOIN EXACTUS.DIGEMA.NOMINA N WITH(NOLOCK) ON N.NOMINA=NH.NOMINA AND N.NOMINA='NOCT' AND NH.FECHA_APROBACION IS NOT NULL " +
                                "ORDER BY FECHA_FIN DESC)) AND (C.CONCEPTO LIKE '%BN%' OR " +
                                "C.CONCEPTO LIKE '%DN%'))C1 WHERE C1.EMPLEADO=EMP.EMPLEADO AND (C1.HORAS > 0 OR C1.TOTAL > 0) AND C1.CONCEPTO LIKE 'B%') " +
                                "End " +
                                "AS TOTAL_BENEFICIOS " +
                                "FROM EXACTUS.DIGEMA.EMPLEADO EMP)C1 " +
                                "WHERE C1.EMPLEADO='" & emplea2_encontrado & "'"
            ...
            For z = 0 To dsconsulta.Tables(0).Rows.Count - 1
                ...
                salario_quincenal = dsconsulta.Tables(0).Rows(z).Item("TOTAL")
            ...
            If (txtsolicitado.Text < 30000 And txtcuotas.Text > 2) Or salario_quincenal < 105000 Then

                If salario_quincenal < 105000 Then
                    MsgBox("Por ley de usura no podemos de momento registrar su solicitud, favor revisar con RRHH al Whatsapp", MsgBoxStyle.Critical, "Construplaza")
                Else
                    MsgBox("Montos menores o iguales a 30 mil solo en dos cuotas, favor revisar", MsgBoxStyle.Critical, "Construplaza")
            ...
            Else
                ...
                If txtcuotas.Text > 0 And txtsolicitado.Text >= 1 And txtsolicitado.Text <= total_disponible Then

                    '    MsgBox("entre")


                    strconexionex = "Data Source=192.168.101.216\BDAPP02;Initial Catalog=coviaseconstruplaza;User Id=sa;Password=Igoloncet!2"
                    Try
                        conexion = New SqlClient.SqlConnection
                        conexion.ConnectionString = strconexionex
                        conexion.Open()
                    ...
                    '    New SqlConnection("Data Source=(192.168.101.15); Initial Catalog=coviaseconstruplaza; user id=sa; password=Igoloncet!2")
                    'ConexionADO.Open()

                    '   MsgBox("CONECTE")

                    Dim ComandoADO As SqlCommand = _
                        New SqlCommand("[dbo].[Usp_GenerarSolicitudCredito]", conexion)
                    ComandoADO.CommandType = CommandType.StoredProcedure

                    Dim paramempleado As SqlParameter = _
                        ComandoADO.Parameters.Add("@cidasociad", SqlDbType.VarChar, 20)
                    paramempleado.Value = txtempleado.Text


                    Dim parammonto As SqlParameter = _
                    ComandoADO.Parameters.Add("@pmonto", SqlDbType.VarChar, 20)
                    parammonto.Value = txtsolicitado.Text


                    Dim paramplazo As SqlParameter = _
                    ComandoADO.Parameters.Add("@pplazo", SqlDbType.VarChar, 20)
                    paramplazo.Value = txtcuotas.Text

                    Dim paramlinea As SqlParameter = _
                    ComandoADO.Parameters.Add("@ccodigolin", SqlDbType.VarChar, 20)
                    paramlinea.Value = "0000000001"

                    Dim paramcomentario As SqlParameter = _
                    ComandoADO.Parameters.Add("@ccomentari", SqlDbType.VarChar, 200)
                    paramcomentario.Value = "desde_kiosco"

                    'output


                    Dim paramResultado As SqlParameter = _
                    ComandoADO.Parameters.Add("@pResultado", SqlDbType.Char, 1)
                    'ComandoADO.Parameters("@pResultado").Direction = ParameterDirection.Output
                    'Resultado = ComandoADO.Parameters("@pResultado").Value.ToString()
                    '  Resultado = paramResultado.Value.ToString()
                    paramResultado.Value = Resultado
                    ' Resultado = paramResultado.Value





                    Dim parammensaje As SqlParameter = _
                     ComandoADO.Parameters.Add("@cMensajeExc", SqlDbType.VarChar, 2000)
                    ' ComandoADO.Parameters("@cMensajeExc").Direction = ParameterDirection.Output
                    'ComandoADO.Parameters("@cMensajeExc").Direction = ParameterDirection.Output
                    'mensa = ComandoADO.Parameters("@cMensajeExc").Value.ToString()
                    'mensa = ComandoADO.Parameters("@cMensajeExc").Value
                    parammensaje.Value = mensa
                    'mensa = parammensaje.Value
                    ...
                    Dim BANDERA_MENSAJE As Boolean = False
                    ...
                    With cmdconsulta
                        .Connection = conexion
                        .CommandText = "SELECT TOP 1 cMensajeExc AS MENSAJE FROM [DIGEMA0020050\BDAPP02].[coviaseconstruplaza].[DBO].geLogExcAG WHERE dFechaCrea >=DATEADD(MINUTE,-1,GETDATE())"
                    ...
                    For z = 0 To dsconsulta.Tables(0).Rows.Count - 1
                        MENSAJE_DE_ERROR = (dsconsulta.Tables(0).Rows(z).Item("MENSAJE"))
                        BANDERA_MENSAJE = True
                        MsgBox(MENSAJE_DE_ERROR, MsgBoxStyle.Exclamation, "Construplaza")
                        Me.Close()
                        Formmenu.btncreditos.Enabled = True
                    ...
                    If BANDERA_MENSAJE = False Then

                        MsgBox("Su Solicitud de Crédito se ha registrado con exito")
                ...
                If txtcuotas.Text = 0 Then
                    MsgBox("Debe definir un número de cuotas", MsgBoxStyle.Critical, "Construplaza")
                End If
                ...
                If txtsolicitado.Text > total_disponible Then
                    MsgBox("Esta solicitando un monto mayor a su disponible " & total_disponible, MsgBoxStyle.Critical, "Construplaza")
                End If
        ...
        Catch ex As Exception
            MsgBox("No se logró insertar, intente nuevamente por favor &", MsgBoxStyle.Critical, "Construplaza")
            m_FPM.CloseDevice()
            Me.Close()
            Formmenu.btncreditos.Enabled = True
            ' Show the stack trace, which is a list of methods 
