...
Public Class Frmaguinaldo
    ...
    Private Sub Frmaguinaldo_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            Formmenu.btnaguinaldo.Enabled = False
                ...
                If emplea2_encontrado <> "NO" Then
                    ...
                    With cmdconsulta
                        ...
                        .CommandText = "SELECT E.NOMBRE " +
                                        ...
                                        "INNER JOIN EXACTUS.DIGEMA.NOMINA N WITH(NOLOCK) ON N.NOMINA='AGUI'" +
                    ...
                    ListView1.Columns.Add("TOTAL", 180, HorizontalAlignment.Left)
                    ...
                    With cmdconsulta
                        .Connection = conexion
                        .CommandText = "SELECT ECN.EMPLEADO,E.NOMBRE,U.DESCRIPCION AS UBICACION,ECN.CONCEPTO,C.DESCRIPCION AS DESCRIPCION,0 as HORAS,case when ECN.CONCEPTO like 'D%'  then ECN.TOTAL * -1 else ECN.TOTAL end AS TOTAL,NH.FECHA_INICIO,NH.FECHA_FIN " +
                                        "FROM EXACTUS.DIGEMA.EMPLEADO_CONC_NOMI ECN " +
                                        "INNER JOIN EXACTUS.DIGEMA.NOMINA_HISTORICO NH WITH(NOLOCK) ON NH.NUMERO_NOMINA=ECN.NUMERO_NOMINA AND NH.NOMINA=ECN.NOMINA " +
                                        "INNER JOIN EXACTUS.DIGEMA.EMPLEADO E WITH(NOLOCK) ON E.EMPLEADO=ECN.EMPLEADO " +
                                        "INNER JOIN EXACTUS.DIGEMA.UBICACION U  WITH(NOLOCK) ON E.UBICACION=U.UBICACION " +
                                        "INNER JOIN EXACTUS.DIGEMA.CONCEPTO C  WITH(NOLOCK) ON C.CONCEPTO=ECN.CONCEPTO " +
                                        "WHERE ECN.EMPLEADO='" & emplea2_encontrado & "' AND ECN.NOMINA='AGUI' " +
                                        "AND ECN.NUMERO_NOMINA=(SELECT TOP 1 NUMERO_NOMINA FROM EXACTUS.DIGEMA.NOMINA_HISTORICO WHERE NOMINA='AGUI' ORDER BY NUMERO_NOMINA DESC)"
                    ...
                    With cmdconsulta_documentos
                        .Connection = conexion
                        .CommandText = "SELECT CEILING(SUM(case when ECN.CONCEPTO like 'D%'  then ECN.TOTAL * -1 else ECN.TOTAL end)) AS TOTAL " +
                                        "FROM EXACTUS.DIGEMA.EMPLEADO_CONC_NOMI ECN " +
                                        "INNER JOIN EXACTUS.DIGEMA.NOMINA_HISTORICO NH WITH(NOLOCK) ON NH.NUMERO_NOMINA=ECN.NUMERO_NOMINA AND NH.NOMINA=ECN.NOMINA " +
                                        "INNER JOIN EXACTUS.DIGEMA.EMPLEADO E WITH(NOLOCK) ON E.EMPLEADO=ECN.EMPLEADO " +
                                        "INNER JOIN EXACTUS.DIGEMA.UBICACION U  WITH(NOLOCK) ON E.UBICACION=U.UBICACION " +
                                        "INNER JOIN EXACTUS.DIGEMA.CONCEPTO C  WITH(NOLOCK) ON C.CONCEPTO=ECN.CONCEPTO " +
                                        "WHERE ECN.EMPLEADO='" & emplea2_encontrado & "' AND ECN.NOMINA='AGUI' AND (ECN.CONCEPTO='BA01' OR ECN.CONCEPTO LIKE 'D%') " +
                                        "AND ECN.NUMERO_NOMINA=(SELECT TOP 1 NUMERO_NOMINA FROM EXACTUS.DIGEMA.NOMINA_HISTORICO WHERE NOMINA='AGUI' ORDER BY NUMERO_NOMINA DESC)"
                    ...
                    For z = 0 To dsconsulta_documentos.Tables(0).Rows.Count - 1



                        txttotales.Text = CDbl(dsconsulta_documentos.Tables(0).Rows(z).Item("TOTAL")).ToString("N2")
            ...
            If bandera_error = True Then
                ...
                m_FPM.CloseDevice()
                Me.Close()
                Formmenu.btnaguinaldo.Enabled = True
